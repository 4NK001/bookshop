...
session_start();
  ...
  if (!isset($_SESSION['userid'])) {
    ...
    $sql = "SELECT tbl_book.bookid, tbl_book.title, tbl_book.authorid, tbl_book.description, tbl_book.categoryid, tbl_book.image, tbl_book.bookfile, tbl_history.page_no FROM tbl_book inner join tbl_history on tbl_book.bookid=tbl_history.bookid and tbl_history.userid=$userid";
$rslt = $conn->query($sql);
...
if ($rslt->num_rows > 0) {
    while ($row = $rslt->fetch_assoc()) {
       ...
       echo '<br>';
       echo '<a href="countinuereading_pdf.php?pdf=' . urlencode($row['bookfile']) . 
       '&page=' . urlencode($row['page_no']) . '&id=' . urlencode($row['bookid']) . '" class="btn">continue</a>';
 echo '</div>';
    }
} else {
    echo "No books found.";
