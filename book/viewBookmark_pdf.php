...
//     $checkSql = "SELECT page_no FROM tbl_bookmark WHERE user_id = '$userId' AND book_id = '$bookId'";
//     $result = $conn->query($checkSql);

//     if ($result) {
//         if ($row = $result->fetch_assoc()) {
//             // Record exists, get the saved page number
//             $pageNum = intval($row['page_no']);
//         } 
//         // else {
//         //     // Insert the new record if not found
//         //     $insertSql = "INSERT INTO tbl_history (reader_book_id, userid, bookid, page_no) VALUES ('$bookId', '$userId', '$bookId', 1)";
...
<div id="review-modal" style="display: none;">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const initialPageNum = <?php echo $pageNum; ?>; // Page number from PHP
            
            if (!pdfUrl || !bookId) {
                console.error('PDF URL or Book ID is missing.');
                return;
            }
            ...
            document.getElementById('next-page').addEventListener('click', () => {
                if (pdfDoc && pageNum < pdfDoc.numPages) {
                    ...
                    // Send AJAX request to update the page number in the history table
            ...
            document.querySelector('#pdf-container img[alt="Bookmark"]').addEventListener('click', () => {
                ...
                if (bookmarkTitle) {
                    ...
                    // Send the request with the book ID, page number, and title
